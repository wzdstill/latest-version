...
\end{description}
\begin{flushleft}
\textbf{Correctness.} We say the IPE scheme described above is correct, if for any $(\msk, \pp) \leftarrow \setup(1^\secparam)$, any message $\mu$, predicate vector $\vec{v}$, and any attribute vector $\vec{w}$ such that $\langle \vec{v}, \vec{w}\rangle = 0$, we have $\dec(\sk_{\vec{v}}, \ct_{\vec{w}}) = \mu$, where $\sk_{\vec{w}} \leftarrow \keygen(\msk, \vec{v})$ and $\ct_{\vec{v}} \leftarrow \enc(\pp, \vec{w}, \mu)$.
\end{flushleft}
\textbf{Security.} For the security definition of IPE, we use the following experiment to describe it. Formally, for any $\ppt$ adversary $\A$, we consider the experiment $\Expt_{\A}^{\mathsf{IPE}}(1^\secparam)$:
\begin{itemize}
...
Regarding the norm defined above, the following bounds hold:
\begin{itemize}
 \item[$\bullet$] Let $\mat{R} \in \{-1, 1\}^{m \times m}$ be chosen at random, then we have $\prob[||\mat{R}|| > 12 \sqrt{2m}] < e^{-2m}$.
 \item[$\bullet$] Let $\mat{R}$ be sampled from $\D_{\Z^{m \times m}, \sigma}$, then we have $\prob [||\mat{R}|| > \sigma \sqrt{m}] < e^{-2m}$.
\end{itemize}
\end{lemma}
...
\begin{itemize}
 \item[$\bullet$] There is an efficient quantum algorithm that solves $\Gapsvp_{\tilde{O}(nq/B)}$ on any $n$-dimensional lattice.
 \item[$\bullet$] If $q\geq \tilde{O}(2^{n/2})$, then there is an efficient classical algorithm for $\Gapsvp_{\tilde{O}(nq/B)}$ on any $n$-dimensional lattice.
\end{itemize}
...
\begin{itemize}
 \item[$\bullet$] There is a $\ppt$  algorithm $\samplepre(\mat{A}, \mat{T}_{\mat{A}}, \vec{u}, \sigma)$: It takes as input
\begin{itemize}
...
then outputs a vector $\vec{x} \in \Lambda^{\perp}_{q}(\mat{A})$ distributed statistically close to $\mathcal{D}_{\Lambda^{\vec{u}}_{q}(\mat{A}),\sigma}$, whenever $\Lambda^{\vec{u}}_{q}(\mat{A})$ is not empty.\\
 \item[$\bullet$] There is a deterministic polynomial algorithm $\extb(\mat{S}, \mat{A}^{'}=\mat{A}\| \bar{\mat{A}})$ : It takes as input
\begin{itemize}
...
then outputs a basis $\mat{S}^{'}$ of $\Lambda^{\bot}(\mat{A}^{'}) \subseteq \Z^{m+\bar{m}}$ such that $\|\tilde{\mat{S}^{'}}\|=\|\tilde{\mat{S}}\|$. Moreover, the same holds even for any given permutation of the columns of $\mat{A}^{'}$(e.g., if columns of $\bar{\mat{A}}$ are both appended and prepended to $\mat{A}$).\\
 \item[$\bullet$] There is a $\ppt$ algorithm $\randb(\mat{S}, s)$ :It takes as input
\begin{itemize}
...
With overwhelming probability, it outputs a basis $\mat{S}^{'}$ of $\Lambda$, which is (essentially) statistically independent of the original basis, and $\| \mat{S}^{'} \|\leq s\sqrt{m}$.\\
 \item[$\bullet$] There is a $\ppt$ algorithm $\sampleleft(\mat{A}, \mat{B}, \mat{T}_{\mat{A}}, \vec{u}, s)$: It takes as input
\begin{itemize}
...
then outputs a vector $\vec r\in\Z^{m+m_1}$ distributed statistically close to $\D_{\Lambda_q^{\vec{u}}(\mat{F}), s}$ where $\mat{F}:=(\mat{A}|\mat{B}).$\\
 \item[$\bullet$] There is a $\ppt$ algorithm $\sampleright(\mat{A}, \mat{B}, \mat{R}, \mat{T}_{\mat{B}}, \vec{u}, s)$: It takes as input
\begin{itemize}
